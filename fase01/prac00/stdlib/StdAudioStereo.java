...
/**
 *  <p><b>Overview.</b>
 *  The {@code StdAudio} class provides a basic capability for playing,
 *  reading, and saving audio. It uses a simple audio model that allows
 *  you to send one sample to the sound card at a time. Each sample is a
...
public final class StdAudioStereo {
    ...
    private static final int MONO   = 1;    // 1 channel
...
*/
     ...
     * @throws IllegalArgumentException if the sampleLeft is {@code Double.NaN}
     * @throws IllegalArgumentException if the sampleRight is {@code Double.NaN}
     */
    ...
    // helper method to the the left or right channel from the given audio file and return as a double[]
    ...
    /**
     * Saves the double array as an monaural audio file (in WAV, AU, or AIFF format).
    ...
    /**
     * Saves the double array as a stereo audio file (using WAV, AU, or AIFF format).
     * The file extension type must be either {@code .wav}, {@code .au},
     * or {@code .aiff}.
     * The audio data format uses a sampling rate of 44,100 Hz, 16-bit audio,
     ...
     *
     * @param  filename the name of the audio file
     * @param  samples the array of samples in left channel
     * @param  samples the array of samples in right channel
     * @throws IllegalArgumentException if unable to save {@code filename}
     * @throws IllegalArgumentException if {@code samplesLeft} is {@code null}
     * @throws IllegalArgumentException if {@code samplesRight} is {@code null}
     * @throws IllegalArgumentException if {@code samplesLeft.length != samplesRight.length}
    ...
    /**
     * Returns the array of samples that was recorded in the left channel
     * since the last call to {@link startRecording()}.
     *
     * @return the array of samples
     * @throws IllegalArgumentException {@code startRecording} was not previously called
     */
    public static double[] getRecordingLeft() {
        if (recordedSamplesLeft != null) {
            double[] results = recordedSamplesLeft.toArray();
            return results;
         }
         else {
             throw new IllegalStateException("no samples were recorded");
         }
    }

    /**
     * Returns the array of samples that was recorded in the right channel
     * since the last call to {@link startRecording()}.
     *
     * @return the array of samples
     * @throws IllegalArgumentException {@code startRecording} was not previously called
    ...
    public static double[] getRecordingRight() {
        if (recordedSamplesRight != null) {
            double[] results = recordedSamplesRight.toArray();
            return results;
         }
         else {
             throw new IllegalStateException("no samples were recorded");
    ...
    ***************************************************************************/
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i <= StdAudioStereo.SAMPLE_RATE; i++) {
            StdAudioStereo.play(0.5 * Math.sin(2*Math.PI * freq * i / StdAudioStereo.SAMPLE_RATE));
        }
        ...
        StdAudioStereo.play(base + "test-22050.wav");    // twenty-four
        StdAudioStereo.play(base + "test.midi");         // a Mozart measure

        // read and play some sound files
        StdAudioStereo.play(StdAudioStereo.read(base + "test.wav"));
        StdAudioStereo.play(StdAudioStereo.read(base + "test-22050.wav"));
        StdAudioStereo.play(StdAudioStereo.read(base + "test.midi"));
